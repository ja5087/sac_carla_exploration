...
class PaddleSAC(parl.Algorithm):
    ...
    def _critic_learn(self, obs, action, reward, next_obs, terminal):
        ...
        # With ReDQ-style algorithms the target is computed from a min of subsets
        with paddle.no_grad():
            next_action, next_log_pro = self.sample(next_obs)
            subset_qt_values = self.target_model.critic_model(
                next_obs, next_action, subset_size=self.subset_size, randomize=True)
            target_Q = paddle.minimum(
                *subset_qt_values) - self.alpha * next_log_pro
            terminal = paddle.cast(terminal, dtype='float32')
            target_Q = reward + self.gamma * (1. - terminal) * target_Q
        all_q_values = self.model.critic_model(obs, action)

        critic_loss = sum(F.mse_loss(qv, target_Q) for qv in all_q_values)
    ...
    def _actor_learn(self, obs):
        ...
        if self.alg == "sac":
            ...
            q_pi = paddle.minimum(q1_pi, q2_pi)
        else:
            # ReDQ and variants actually use the mean over all Q functions idek why
            all_q_values = self.model.critic_model(obs, act)
            q_pi = sum(all_q_values) / len(all_q_values)
