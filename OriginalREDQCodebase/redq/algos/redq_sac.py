...
from redq.algos.core import (Mlp, ReplayBuffer, TanhGaussianPolicy,
...
class REDQSACAgent(object):
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 ...
                 # added 20211206
                 target_drop_rate=0.0, layer_norm=False, spectral_norm=False, l1=0.0, l2=0.0
                 ):
        # set up networks
        self.policy_net = TanhGaussianPolicy(
            obs_dim, act_dim, hidden_sizes, action_limit=act_limit).to(device)
        self.q_net_list, self.q_target_net_list = [], []
        for q_i in range(num_Q):
            # new_q_net = Mlp(obs_dim + act_dim, 1, hidden_sizes).to(device)
            new_q_net = Mlp(obs_dim + act_dim, 1, hidden_sizes,
                            target_drop_rate=target_drop_rate, layer_norm=layer_norm, spectral_norm=spectral_norm).to(device)
        ...
        for q_i in range(num_Q):
            self.q_optimizer_list.append(optim.Adam(
                self.q_net_list[q_i].parameters(), lr=lr, weight_decay=l2))
    ...
    def get_ave_q_prediction_for_bias_evaluation(self, obs_tensor, acts_tensor):
        ...
        for q_i in range(self.num_Q):
            q_prediction = self.q_net_list[q_i](
                torch.cat([obs_tensor, acts_tensor], 1))
    ...
    def get_redq_q_target_no_grad(self, obs_next_tensor, rews_tensor, done_tensor):
        ...
        sample_idxs = np.random.choice(
            self.num_Q, num_mins_to_use, replace=False)
        with torch.no_grad():
            if self.q_target_mode == 'min':
                ...
                min_q, min_indices = torch.min(
                    q_prediction_next_cat, dim=1, keepdim=True)
                next_q_with_log_prob = min_q - self.alpha * log_prob_a_tilda_next
                y_q = rews_tensor + self.gamma * \
                    (1 - done_tensor) * next_q_with_log_prob
            if self.q_target_mode == 'ave':
                ...
                for q_i in range(self.num_Q):
                    q_prediction_next = self.q_target_net_list[q_i](
                        torch.cat([obs_next_tensor, a_tilda_next], 1))
                    q_prediction_next_list.append(q_prediction_next)
                q_prediction_next_ave = torch.cat(
                    q_prediction_next_list, 1).mean(dim=1).reshape(-1, 1)
            ...
            if self.q_target_mode == 'rem':
                ...
                # apply rem here
                q_prediction_next_cat = torch.cat(q_prediction_next_list, 1)
                rem_weight = Tensor(np.random.uniform(
                    0, 1, q_prediction_next_cat.shape)).to(device=self.device)
                normalize_sum = rem_weight.sum(
                    1).reshape(-1, 1).expand(-1, self.num_Q)
                rem_weight = rem_weight / normalize_sum
                q_prediction_next_rem = (
                    q_prediction_next_cat * rem_weight).sum(dim=1).reshape(-1, 1)
    ...
    def train(self, logger):
        ...
        num_update = 0 if self.__get_current_num_data(
        ) <= self.delay_update_steps else self.utd_ratio
        for i_update in range(num_update):
            ...
            for q_net in self.q_net_list:
                q_loss_all += self.l1 * torch.sum(torch.abs(q_net.last_fc_layer.weight))
            ...
            if ((i_update + 1) % self.policy_update_delay == 0) or i_update == num_update - 1:
                ...
                for sample_idx in range(self.num_Q):
                    ...
                    q_a_tilda = self.q_net_list[sample_idx](
                        torch.cat([obs_tensor, a_tilda], 1))
                ...
                if self.auto_alpha:
                    alpha_loss = - \
                        (self.log_alpha * (log_prob_a_tilda +
        ...
        if num_update == 0:
            logger.store(LossPi=0, LossQ1=0, LossAlpha=0,
