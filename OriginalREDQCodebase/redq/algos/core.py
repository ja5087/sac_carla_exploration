...
mbpo_target_entropy_dict = {'Hopper-v2': -1, 'HalfCheetah-v2': -
                            3, 'Walker2d-v2': -3, 'Ant-v2': -4, 'Humanoid-v2': -2}
mbpo_epoches = {'Hopper-v2': 125, 'Walker2d-v2': 300,
                'Ant-v2': 300, 'HalfCheetah-v2': 400, 'Humanoid-v2': 300}
...
class Mlp(nn.Module):
    def __init__(
        ...
        for i, next_size in enumerate(hidden_sizes):
            ...
            # Per Gogianu et al. SN should be applied on the second-last layer only
            if spectral_norm and i == len(hidden_sizes) - 1:
                fc_layer = nn.utils.parametrizations.spectral_norm(fc_layer)
            ...
            if target_drop_rate > 0.0:
                self.hidden_layers.append(
                    nn.Dropout(p=target_drop_rate))  # dropout
            if layer_norm:
                self.hidden_layers.append(nn.LayerNorm(
                    fc_layer.out_features))  # layer norm
